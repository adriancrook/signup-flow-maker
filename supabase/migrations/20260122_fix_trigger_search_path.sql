...
begin
  ...
  select id into org_id from public.organizations where lower(domain) = user_domain;
  
  -- If not, and it's teaching.com, create it (First user provision)
  if org_id is null and user_domain = 'teaching.com' then
    -- Try to find it by slug first
    select id into org_id from public.organizations where slug = 'teaching-com';
